...
The business-card branch `claude/business-nfc-card-design-4d2727` is **being merged into `main`
now**: the mark engraved on the card plate (D-051), the plate recomposition (D-052), the mark on
the print fronts (D-053), and the founder's full-metallic override with per-person phone numbers
and the NFC hosting decision (D-054). Everything else is already merged and live — the logo mark
(D-050), the NB-VK digital cards (D-049), the animated wordmark sting (D-046) and the wordmark
drop (D-047). `main` stays the only long-lived branch.
